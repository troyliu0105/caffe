...
public:
  Detector(const string &model_file, const string &weights_file,
           ...
           const float confidence_threshold, const float normalize_value,
           const string &cpu_mode, const int resize);

  std::vector<vector<float>> Detect(cv::Mat &img);
  std::vector<vector<float>>
  DetectAndSegment(cv::Mat &img, std::vector<cv::Mat> &seg_img, int scale = 4);
  std::vector<vector<float>>
  Segment(cv::Mat &img, std::vector<cv::Mat> &seg_img, int scale = 4);
...
private:
  ...
  cv::Mat Preprocess(const cv::Mat &img, std::vector<cv::Mat> *input_channels);
  cv::Mat Preprocess(const cv::Mat &img, std::vector<cv::Mat> *input_channels,
...
Detector::Detector(const string &model_file, const string &weights_file,
                   ...
                   const float normalize_value, const string &cpu_mode,
                   const int resize) {
  if (cpu_mode == "cpu") {
  ...
  // CHECK_EQ(net_->num_outputs(), 1) << "Network should have exactly one
  // output.";
...
std::vector<vector<float>>
Detector::Segment(cv::Mat &img, std::vector<cv::Mat> &seg_img, int scale) {
  Blob<float> *input_layer = net_->input_blobs()[0];
  input_layer->Reshape(1, num_channels_, input_geometry_.height,
                       input_geometry_.width);
  /* Forward dimension change to all layers. */
  ...
  vector<vector<float>> detections;
  //
  // seg_img = cv::Mat(input_geometry_.width / 8, input_geometry_.height / 8,
  // CV_8UC1);
  int img_index1 = 0;
  int w = input_geometry_.width / scale;
  int h = input_geometry_.height / scale;
  for (int i = 0; i < size; i++) {
    seg_img.push_back(cv::Mat(input_geometry_.width / scale,
                              input_geometry_.height / scale, CV_8UC1));
  }
  // printf("%d\n",seg_img.size());
    ...
    for (int y = 0; y < h; y++) {
      ...
      for (int j = 0; j < w; j++) {
        ptr2[img_index2] = (unsigned char)BOUND(
            (result[img_index1 + c * w * h]) * 255, 0, 255);
        // if(c==1)
...
std::vector<vector<float>>
Detector::DetectAndSegment(cv::Mat &img, std::vector<cv::Mat> &seg_img,
                           int scale) {
  Blob<float> *input_layer = net_->input_blobs()[0];
  input_layer->Reshape(1, num_channels_, input_geometry_.height,
                       input_geometry_.width);
  ...
  for (int c = 0; c < seg_img.size(); c++) {
    ...
    for (int y = 0; y < h; y++) {
      ...
      for (int j = 0; j < w; j++) {
        ptr2[img_index2] = (unsigned char)BOUND(
            (result2[img_index1 + c * w * h]) * 255, 0, 255);
        // if(c==1)
        //  printf("%f\n",result2[img_index1+c*w*h]);
        img_index1++;
        img_index2++;
...
cv::Mat Detector::Preprocess(const cv::Mat &img,
  ...
  if (sample.size() != input_geometry_) {
    if (resize_mode == 1) {
      sample_resized = LetterBoxResize(sample, input_geometry_.width,
...
void MatMul(cv::Mat img1, vector<cv::Mat> img2, vector<int> color,
            bool show_mode = false) {
  int i, j;
  int height = img1.rows;
  int width = img1.cols;
  // LOG(INFO) << width << "," << height << "," << img1.rows << "," <<
  // img1.cols; LOG(INFO) <<  "-------------------------------------"; #pragma
  // omp parallel for
  int size = img2.size();
...
void MatMul(cv::Mat img1, cv::Mat img2, int r, int g, int b,
            bool show_mode = false) {
  int i, j;
  int height = img1.rows;
  int width = img1.cols;
  // LOG(INFO) << width << "," << height << "," << img2.rows << "," <<
  // img2.cols; #pragma omp parallel for
  ...
  for (i = 0; i < height; i++) {
    ...
    for (j = 0; j < width; j++) {
      ...
      // ptr1[img_index1+idx] = (unsigned char) BOUND(ptr1[img_index1] +
      // ptr2[img_index2] * 1.0,0,255); ptr1[img_index1+1] = (ptr2[img_index2]);
      // ptr1[img_index1+2] = (unsigned char) BOUND(ptr1[img_index1+2] +
      // (255-ptr2[img_index2]) * 0.4,0,255); ptr1[img_index1+2] = (unsigned
      // char) BOUND((ptr2[img_index2]) ,0,255);
      img_index1 += 3;
      img_index2++;
    }
  }
...
DEFINE_int32(wait_time, 1000, "cv imshow window waiting time ");
DEFINE_int32(resize_mode, 0, "0:WARP , 1:FIT_LARGE_SIZE_AND_PAD");
DEFINE_string(cpu_mode, "cpu", "cpu , gpu");
DEFINE_int32(detect_mode, 0, "0: detect , 1:segementation");
DEFINE_string(indir, "data//", "demo images folder");
DEFINE_string(ext, "jpg", "demo images extension");
DEFINE_string(seg_label_map, "data/cityscapes/labelmap_seg.prototxt",
              "label map");
DEFINE_int32(seg_resize_scales, 4, "segmentation output image size scale");
int main(int argc, char **argv) {
  ...
  Detector detector(model_file, weights_file, mean_file, mean_value,
                    confidence_threshold, normalize_value, cpu_mode,
                    resize_mode);
  ...
  if (detect_mode) {
    ...
    for (int i = 0; i < detector.label_map_.item().size(); i++) {
      if (detector.label_map_.item(i).label_id()) {
        // LOG(INFO)<<detector.label_map_.item(i).name() << ","
        // <<detector.label_map_.item(i).label_id()<< ","
        // <<detector.label_map_.item(i).label()
        //<< " , RGB :" << detector.label_map_.item(i).r()<< ","<<
        // detector.label_map_.item(i).g()<< ","<<
        // detector.label_map_.item(i).b();
  ...
  if (file_type == "image") {
    ...
    for (size_t k = 0; k < fn.size(); ++k) {
      ...
      // cv::Mat seg_img(detector.input_geometry_.width/8,
      // detector.input_geometry_.height/8, CV_8UC1);
      ...
      if (detect_mode == 2) {
        ...
        for (int i = 0; i < seg_img.size(); i++) {
          cv::resize(seg_img[i], seg_img_resized, cv::Size(img.cols, img.rows),
                     cv::INTER_AREA);
          int color_index = (i + 1) * 3;
          MatMul(img, seg_img_resized, color[color_index],
                 color[color_index + 1], color[color_index + 2]);
        }
      } else if (detect_mode) {
        ...
        // cv::resize(seg_img, seg_img_resized, cv::Size(img.cols,
        // img.rows),cv::INTER_AREA); MatMul(img, seg_img_resized);
      } else {
      ...
      if (detect_mode < 2) {
        for (int i = 0; i < detections.size(); ++i) {
          ...
          if (score >= confidence_threshold) {
            ...
            char label[100];
            sprintf(label, "%s,%f", YOLO_CLASSES[static_cast<int>(d[1])],
                    score);
            int baseline;
            ...
            cv::putText(img, label, pt1, cv::FONT_HERSHEY_SIMPLEX, 0.5,
                        cv::Scalar(0, 0, 0));
          }
        }
      }
      ...
      if (count <= max) {
        ...
        if (count == 0) {
          writer.open("VideoTest.mp4",
                      cv::VideoWriter::fourcc('M', 'P', '4', 'V'), 10, size);
        } else if (count == max) {
          writer << img;
          writer.release();
        } else {
    ...
    for (size_t k = 0; k < fn.size(); ++k) {
      ...
      while (true) {
        ...
        if (detect_mode) {
          seg_img.clear();
          detections =
              detector.DetectAndSegment(img, seg_img, seg_resize_scales);
          for (int i = 0; i < seg_img.size(); i++) {
            cv::resize(seg_img[i], seg_img_resized,
                       cv::Size(img.cols, img.rows), cv::INTER_AREA);
            int color_index = (i)*3;
            MatMul(img, seg_img_resized, color[color_index],
                   color[color_index + 1], color[color_index + 2]);
          }
