...
__global__ void deformable_im2col_gpu_kernel(
  ...
  CUDA_KERNEL_LOOP(index, n) {
    ...
    data_offset_ptr += deformable_group_index * 2 * kernel_h * kernel_w *
                       height_col * width_col; // 0
...
template <typename Dtype>
void deformable_im2col_gpu(const Dtype *data_im, const Dtype *data_offset,
  ...
  deformable_im2col_gpu_kernel<Dtype>
      <<<CAFFE_GET_BLOCKS(num_kernels), CAFFE_CUDA_NUM_THREADS>>>(
          num_kernels, data_im, data_offset, height, width, kernel_h, kernel_w,
...
template void deformable_im2col_gpu<double>(
    const double *data_im, const double *data_offset, const int channels,
    const int height, const int width, const int kernel_h, const int kernel_w,
    const int pad_h, const int pad_w, const int stride_h, const int stride_w,
    const int dilation_h, const int dilation_w, const int deformable_group,
    double *data_col);
    ...
    for (int dy = -2; dy <= 2; dy++) {
      for (int dx = -2; dx <= 2; dx++) {
        if (cur_h + dy >= 0 && cur_h + dy < height && cur_w + dx >= 0 &&
          ...
          caffe_gpu_atomic_add(weight * cur_top_grad,
...
void deformable_col2im_gpu(const Dtype *data_col, const Dtype *data_offset,
                           const int channels, const int height,
                           const int width, const int num_kernels,
  ...
  deformable_col2im_gpu_kernel<Dtype>
      <<<CAFFE_GET_BLOCKS(num_kernels), CAFFE_CUDA_NUM_THREADS>>>(
          num_kernels, data_col, data_offset, channels, height, width, kernel_h,
...
template void deformable_col2im_gpu<float>(
    ...
    const int dilation_w, const int deformable_group, float *grad_im);
template void deformable_col2im_gpu<double>(
    const double *data_col, const double *data_offset, const int channels,
    const int height, const int width, const int num_kernels,
    const int kernel_h, const int kernel_w, const int pad_h, const int pad_w,
    const int stride_h, const int stride_w, const int dilation_h,
    const int dilation_w, const int deformable_group, double *grad_im);
...
void deformable_col2im_coord_gpu(
  ...
  int width_col =
      (width + 2 * pad_w - (dilation_w * (kernel_w - 1) + 1)) / stride_w + 1;
  int num_kernels =
      height_col * width_col * 2 * kernel_h * kernel_h * deformable_group;
