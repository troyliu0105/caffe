...
void hdf5_load_nd_dataset_helper(hid_t file_id, const char *dataset_name_,
  ...
  if (reshape) {
    ...
    if (blob_dims != blob->shape()) {
      ...
      CHECK(blob_dims == blob->shape())
          << "Cannot load blob from hdf5; shape "
...
void hdf5_save_string(hid_t loc_id, const string &dataset_name,
  ...
  CHECK_GE(status, 0) << "Failed to save string dataset with name "
                      << dataset_name;
}
...
string hdf5_get_name_by_idx(hid_t loc_id, int idx) {
  ...
  ssize_t status =
      H5Lget_name_by_idx(loc_id, ".", H5_INDEX_NAME, H5_ITER_NATIVE, idx, c_str,
                         str_size + 1, H5P_DEFAULT);
