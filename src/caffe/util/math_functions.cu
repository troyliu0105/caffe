...
__global__ void logistic_activate_kernel(const int n, const Dtype *a,
                                         Dtype *y) {
  CUDA_KERNEL_LOOP(index, n) { y[index] = 0.5 * tanh(0.5 * a[index]) + 0.5; }
}
template <>
void caffe_gpu_logistic_activate<float>(const int N, const float *a, float *y) {
  // NOLINT_NEXT_LINE(whitespace/operators)
  logistic_activate_kernel<float>
      <<<CAFFE_GET_BLOCKS(N), CAFFE_CUDA_NUM_THREADS>>>(N, a, y);
}
template <>
void caffe_gpu_logistic_activate<double>(const int N, const double *a,
                                         double *y) {
  // NOLINT_NEXT_LINE(whitespace/operators)
  logistic_activate_kernel<double>
      <<<CAFFE_GET_BLOCKS(N), CAFFE_CUDA_NUM_THREADS>>>(N, a, y);
}
template <typename Dtype>
