...
void Yolov3DetectionOutputLayer<Dtype>::Forward_gpu(const vector<Blob < Dtype> *
...
if(gaussian_box_) {
caffe_gpu_logistic_activate(4 *
side_w_ *side_h_, input_data
+ index,swap_data +index );
index = n * len * stride + b * bottom[0]->count(1) + 4 * stride;
caffe_copy(side_w_
*side_h_, input_data + index, swap_data + index);
index = n * len * stride + b * bottom[0]->count(1) + 5 * stride;
caffe_gpu_logistic_activate(side_w_
*side_h_,input_data + index,swap_data +index );
...
caffe_gpu_logistic_activate((num_class_
+1) *
side_w_ *side_h_, input_data
+ index,swap_data +index );
}
else {
caffe_gpu_logistic_activate(2 *
side_w_ *side_h_, input_data
+ index,swap_data +index );
...
Dtype obj_score;
if(gaussian_box_) {
Dtype uc_ver = 4.0 - swap_data[index + 1 * stride] - swap_data[index + 1 * stride] - swap_data[index + 3 * stride] -
...
template<typename Dtype>
void Yolov3DetectionOutputLayer<Dtype>::Backward_gpu(const vector<Blob < Dtype> *
