...
template <typename Dtype>
void AssistedExcitationLayer<Dtype>::visualization(
    const vector<Blob<Dtype> *> &bottom, const vector<Blob<Dtype> *> &top) {
...
template <typename Dtype>
void AssistedExcitationLayer<Dtype>::Forward_cpu(
    const vector<Blob<Dtype> *> &bottom, const vector<Blob<Dtype> *> &top) {
  ...
  float alpha =
      BOUND(cos((this->iter_ / (float)(this->max_iter_ * 0.75)) * M_PI / 2.0f),
            0, 1.0f);
  // LOG(INFO) << alpha ;
...
template <typename Dtype>
void AssistedExcitationLayer<Dtype>::Backward_cpu(
