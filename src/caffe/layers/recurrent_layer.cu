...
void RecurrentLayer<Dtype>::Forward_gpu(const vector<Blob < Dtype> *
>& bottom,
...
for (
int i = 0;
i<recur_input_blobs_.
size();
++i) {
const int count = recur_input_blobs_[i]->count();
DCHECK_EQ(count, recur_output_blobs_[i]
->
count()
);
...
unrolled_net_->
ForwardTo(last_layer_index_);

if (expose_hidden_) {
const int top_offset = output_blobs_.size();
for (
int i = top_offset, j = 0;
i<top.
size();
++i, ++j) {
top[i]->
ShareData(*recur_output_blobs_[j]);
}
}
}
