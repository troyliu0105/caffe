...
void ClipLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype> *
> &bottom,
  ...
  for (
      ...
      ++i) {
    top_data[i] =
        std::max(min, std::min(bottom_data[i], max)
        );
  }
}

template<typename Dtype>
void ClipLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype> *
> &top,
