...
template<typename Dtype>
void NormalizeLayer<Dtype>::Forward_gpu(const vector<Blob < Dtype> *
...
caffe_gpu_set<Dtype>(norm_
.

count(), Dtype(eps_), norm_data
...
template<typename Dtype>
void NormalizeLayer<Dtype>::Backward_gpu(const vector<Blob < Dtype> *
...
++n) {
// compute a
caffe_gpu_mul<Dtype>(dim, top_data
+
n *dim, top_diff
+
n *dim, buffer_data
...
caffe_gpu_scale<Dtype>(dim, a
/ norm_data[n] / norm_data[n],
