...
__global__ void reorg_kernel(const Dtype *x, int w, int h, int c, int batch,
                             int stride, int forward, Dtype *out) {
...
void ReorgLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype> *> &bottom,
  ...
  reorg_kernel<Dtype><<<CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS>>>(
