...
void ShuffleChannelLayer<Dtype>::Forward_cpu(
  ...
  for (int n = 0; n < num; ++n) {
    Resize_cpu(top_data + n * feature_map_size,
               bottom_data + n * feature_map_size, group_row, group_column,
               sp_sz);
  }
...
void ShuffleChannelLayer<Dtype>::Backward_cpu(
  ...
  if (propagate_down[0]) {
    ...
    for (int n = 0; n < num; ++n) {
      Resize_cpu(bottom_diff + n * feature_map_size,
                 top_diff + n * feature_map_size, group_row, group_column,
                 sp_sz);
    }
