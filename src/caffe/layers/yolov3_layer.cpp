...
dxrep dx_box_iou(vector<Dtype> pred, vector<Dtype> truth, IOU_LOSS iou_loss) {
  ...
  ddx.dt =
      p_dx;      //We follow the original code released from GDarknet. So in yolo_layer.c, dt, db, dl, dr are already dx, dy, dw, dh.
  ddx.db = p_dy;
  ddx.dl = p_dw;
  ddx.dr = p_dh;
...
void Yolov3Layer<Dtype>::LayerSetUp(
  ...
  int tmp_input_count =
      side_w_ * side_h_ * num_ * (4 + num_class_ + 1); //13*13*5*(20+4+1) label: isobj, class_label, coordinates
  ...
  if (!(iter_ % 16)) {
    if (time_count_ > 0) {
      LOG(INFO) << "noobj: " << score_.avg_anyobj / time_count_ << " obj: " << score_.avg_obj / time_count_ <<
                " iou: " << score_.avg_iou / time_count_ << " cat: " << score_.avg_cat / time_count_ << " recall: "
                << score_.recall / time_count_ << " recall75: " << score_.recall75 / time_count_ << " count: "
                << class_count_ / time_count_;
