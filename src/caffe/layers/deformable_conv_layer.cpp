...
template <typename Dtype>
void DeformableConvolutionLayer<Dtype>::Reshape(
  ...
  this->output_offset_ =
      this->conv_out_channels_ * this->conv_out_spatial_dim_ / this->group_;
  ...
  this->num_kernels_im2col_ =
      this->conv_in_channels_ * this->conv_out_spatial_dim_;
  this->num_kernels_col2im_ =
      reverse_dimensions() ? this->top_dim_ : this->bottom_dim_;
...
template <typename Dtype>
void DeformableConvolutionLayer<Dtype>::LayerSetUp(
    const vector<Blob<Dtype> *> &bottom, const vector<Blob<Dtype> *> &top) {

  // Configure the kernel size, padding, stride, and inputs.
  DeformableConvolutionParameter conv_param =
      this->layer_param_.deformable_convolution_param();
  ...
  this->num_output_ =
      this->layer_param_.deformable_convolution_param().num_output();
  CHECK_GT(this->num_output_, 0);
  this->group_ = this->layer_param_.deformable_convolution_param().group();
  this->deformable_group_ =
      this->layer_param_.deformable_convolution_param().deformable_group();
  CHECK_EQ(this->channels_ % this->group_, 0);
  CHECK_EQ(this->num_output_ % this->group_, 0)
      << "Number of output should be multiples of group.";

  CHECK_EQ(bottom[1]->shape(1), kernel_shape_data[0] * kernel_shape_data[1] *
                                    this->deformable_group_ * 2)
  ...
  this->bias_term_ =
      this->layer_param_.deformable_convolution_param().bias_term();
