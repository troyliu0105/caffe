...
void AssistedExcitationLayer<Dtype>::Forward_gpu(const vector<Blob < Dtype> *
...
caffe_gpu_axpy(width
*height, Dtype(1) , mask_data, &top_data[offset]);
}
/*if(b==0) {
...
template<typename Dtype>
void AssistedExcitationLayer<Dtype>::Backward_gpu(const vector<Blob < Dtype> *
