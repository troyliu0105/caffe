...
template <typename Dtype>
void Yolov3DetectionOutputLayer<Dtype>::LayerSetUp(
  ...
  CHECK(yolov3_detection_output_param.has_num_classes())
...
template <typename Dtype>
void Yolov3DetectionOutputLayer<Dtype>::Reshape(
    const vector<Blob<Dtype> *> &bottom, const vector<Blob<Dtype> *> &top) {
  ...
  if (num_kept == 0) {
    ...
    for (int i = 0; i < num_kept; i++) {
      ...
      DLOG(INFO) << "Detection box"
                 << "," << predicts_[idxes[i]].classType << ","
                 << predicts_[idxes[i]].x << "," << predicts_[idxes[i]].y << ","
                 << predicts_[idxes[i]].w << "," << predicts_[idxes[i]].h;
    }
  }
}
template <typename Dtype>
