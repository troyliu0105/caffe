...
#include "caffe/layers/loss/contrastive_loss_layer.hpp"
  ...
  for (int i = 0; i < bottom[0]->num(); ++i) {
    if (static_cast<int>(bottom[2]->cpu_data()[i])) { // similar pairs
      ...
      if (legacy_version) {
        ...
        Dtype dist =
            std::max(margin - sqrt(dist_sq_.cpu_data()[i]), Dtype(0.0));
        loss += dist * dist;
      }
    }
  }
...
template <typename Dtype>
__global__ void
CLLBackward(const int count, const int channels, const Dtype margin,
