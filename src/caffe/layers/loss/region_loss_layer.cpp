...
void RegionLossLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype> *> &bottom,
  ...
  // bottom[0]->height() << "," << bottom[0]->width(); int index = 0;
    ...
    for (int s = 0; s < side_ * side_; s++) {
      for (int n = 0; n < num_; n++) {
        ...
        /*if (iter_ < 12800 / bottom[0]->num()) {
          ...
          //LOG(INFO)<<index;
          delta_region_box(truth, swap_data, biases_, n, index, x2, y2, side_,
        side_, diff, .01, stride);
        }*/
      }
    }
    for (int t = 0; t < 300; ++t) {
      ...
      float iou;
      if (rescore_) {
        iou = delta_region_box(truth, swap_data, biases_, best_n, best_index, i,
                               j, side_, side_, diff, coord_scale_, stride);
      } else {
        iou = delta_region_box(
            truth, swap_data, biases_, best_n, best_index, i, j, side_, side_,
            diff, coord_scale_ * (2 - truth[2] * truth[3]), stride);
      }
      if (iou > 0.5)
        recall += 1;
      if (iou > 0.75)
  ...
  if (!(iter_ % 10)) {
    if (time_count_ > 0) {
      LOG(INFO) << "noobj: " << score_.avg_anyobj / 10.
