/**
 * @brief
 * Source: https://github.com/BOBrown/SSD-Centerloss
 */
#include <algorithm>
#include <map>
#include <utility>
...
#include "caffe/layers/loss/multibox_center_loss_layer.hpp"
#include "caffe/util/math_functions.hpp"

namespace caffe {

template <typename Dtype>
void MultiBoxCenterLossLayer<Dtype>::LayerSetUp(
  ...
  CHECK(multibox_center_loss_param.has_center_features())
      << "Must provide center_features.";
  ...
  loc_bottom_vec_.push_back(&loc_pred_); // loc_bottom_vec_[0]���loc_pred_����
  ...
  if (conf_loss_type_ == MultiBoxLossParameter_ConfLossType_SOFTMAX) {
    ...
    CenterLossParameter *center_param =
        center_layer_param.mutable_center_loss_param();
    ...
    conf_center_loss_layer_ =
        LayerRegistry<Dtype>::CreateLayer(center_layer_param);
    conf_center_loss_layer_->SetUp(conf_center_bottom_vec_,
                                   conf_center_top_vec_);

    // holobo:setup SoftmaxWithLoss
    LayerParameter layer_param;
    layer_param.set_name(this->layer_param_.name() + "_softmax_conf");
    layer_param.set_type("SoftmaxWithLoss"); //������Ը�Ϊcenter loss
...
template <typename Dtype>
void MultiBoxCenterLossLayer<Dtype>::Reshape(
    const vector<Blob<Dtype> *> &bottom, const vector<Blob<Dtype> *> &top) {
...
template <typename Dtype>
void MultiBoxCenterLossLayer<Dtype>::Forward_cpu(
    const vector<Blob<Dtype> *> &bottom, const vector<Blob<Dtype> *> &top) {
  const Dtype *loc_data = bottom[0]->cpu_data();
  const Dtype *conf_data = bottom[1]->cpu_data();
  const Dtype *prior_data = bottom[2]->cpu_data();
  const Dtype *gt_data = bottom[3]->cpu_data();
  const Dtype *conf_center_data =
      bottom[4]->cpu_data(); // bottom[4] represents feature sets that calculate
                             // the center.
  ...
  if (num_matches_ >= 1) {
    ...
    loc_pred_.Reshape(loc_shape); // loc_pred_���棺�Ѿ�ƥ������� * 4
    loc_gt_.Reshape(loc_shape); //ͬ��
    Dtype *loc_pred_data =
        loc_pred_.mutable_cpu_data(); //��ȡloc_pred_����,��ָ��ָ��
    Dtype *loc_gt_data = loc_gt_.mutable_cpu_data(); //��ȡgt����
  ...
  if (num_conf_ >= 1) {
    ...
    if (conf_loss_type_ == MultiBoxLossParameter_ConfLossType_SOFTMAX) {
      ...
      conf_gt_.Reshape(conf_center_shape); // conf_gt_ reshape��Ϊ[num_classes_]
      conf_center_shape.push_back(center_features_);
      conf_center_pred_.Reshape(
          conf_center_shape); // conf_center_pred_
                              // reshape��Ϊ[num_conf_,center_features]
    } else if (conf_loss_type_ == MultiBoxLossParameter_ConfLossType_LOGISTIC) {
      ...
      conf_center_shape.push_back(num_conf_);
      conf_center_shape.push_back(
          center_features_); //ע������conf_center_shape��Ҫ��Ϊ[num_conf,center_features]
      conf_gt_.Reshape(conf_center_shape);
      conf_center_pred_.Reshape(conf_center_shape);
    } else {
      LOG(FATAL) << "Unknown confidence loss type.";
    ...
    Dtype *conf_pred_data =
        conf_pred_
            .mutable_cpu_data(); // conf_pred_dataָ��ָ����bottom[1]����������
    Dtype *conf_gt_data =
        conf_gt_.mutable_cpu_data(); // conf_pred_dataָ��ָ����bottom[3]����������
    // center shares the data of bottom[4]
    ...
    for (int i = 0; i < num_; ++i) {
      if (all_gt_bboxes.find(i) != all_gt_bboxes.end()) {
        ...
        const map<int, vector<int>> &match_indices = all_match_indices_
            [i]; // all_match_indices_����������gt��prior_box��ƥ����
        for (int j = 0; j < num_priors_; ++j) {
          for (map<int, vector<int>>::const_iterator it = match_indices.begin();
            ...
            if (do_neg_mining_) {
              ...
              caffe_copy<Dtype>(
                  num_classes_, conf_data + j * num_classes_,
                  conf_pred_data +
                      count *
                          num_classes_); //����conf_data����mbox_layer��bottom[1]

              caffe_copy<Dtype>(
                  center_features_, conf_center_data + j * center_features_,
        ...
        if (do_neg_mining_) {
          ...
          for (int n = 0; n < all_neg_indices_[i].size(); ++n) {
            ...
            caffe_copy<Dtype>(center_features_,
                              conf_center_data + j * center_features_,
                              conf_center_pred_data + count * center_features_);
    ...
    conf_center_loss_layer_->Reshape(conf_center_bottom_vec_,
                                     conf_center_top_vec_);
    conf_center_loss_layer_->Forward(conf_center_bottom_vec_,
                                     conf_center_top_vec_);
  } else {
  ...
  if (this->layer_param_.propagate_down(4)) {
    Dtype normalizer = LossLayer<Dtype>::GetNormalizer(
        normalization_, num_, num_priors_, num_matches_);
    top[0]->mutable_cpu_data()[0] +=
        center_loss_weight_ * conf_center_loss_.cpu_data()[0] / normalizer;
    // LOG(INFO) << "center_loss:" << center_loss_weight_ *
    // conf_center_loss_.cpu_data()[0] / normalizer;
  }
}

template <typename Dtype>
void MultiBoxCenterLossLayer<Dtype>::Backward_cpu(
      ...
      for (int i = 0; i < num_; ++i) {
        for (map<int, vector<int>>::iterator it = all_match_indices_[i].begin();
          ...
          for (int j = 0; j < match_index.size(); ++j) {
            ...
            int start_idx = loc_classes_ * 4 * j +
                            label * 4; //�������share_location_�������4*j
  ...
  if (propagate_down[4]) {
    ...
    if (num_conf_ >= 1) {
      ...
      conf_center_loss_layer_->Backward(
          conf_center_top_vec_, conf_propagate_down, conf_center_bottom_vec_);
      // Scale gradient.
      Dtype normalizer = LossLayer<Dtype>::GetNormalizer(
          normalization_, num_, num_priors_, num_matches_);
      Dtype loss_weight = top[0]->cpu_diff()[0] / normalizer;
        ...
        for (int i = 0; i < num_; ++i) {
          ...
          for (map<int, vector<int>>::const_iterator it = match_indices.begin();
            ...
            for (int j = 0; j < num_priors_; ++j) {
              ...
              // Copy the diff to the right place.
              caffe_copy<Dtype>(center_features_,
                                conf_center_pred_diff +
                                    count * center_features_,
