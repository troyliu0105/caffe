...
__global__ void
ConvForward(const int nthreads, const Dtype *const bottom_data, const int num,
            ...
            const Dtype *const weight, const Dtype *const bias,
  ...
  CUDA_KERNEL_LOOP(index, nthreads) {
    ...
    for (int h = hstart; h < hend; ++h) {
      for (int w = wstart; w < wend; ++w) {
        ...
        //					printf("pos:h%d w%d\n",h,w);
        //					printf("cal:bottom%f
        //weight%f\n",bottom_slice[h * width + w],weight_slice[(h-hstart) *
        //kernel_w + (w-wstart)]);
        //				}
      }
    }
  ...
  for (int i = 0; i < bottom.size(); ++i) {
    ...
    if (bias_term_) {
      ...
      ConvForward<Dtype><<<CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS>>>(
          count, bottom_data, bottom[i]->num(), channels_, height_, width_,
          conved_height, conved_weight, kernel_h_, kernel_w_, stride_h_,
          stride_w_, pad_h_, pad_w_, top_data, weight, bias, bias_term_);
    } else {
      ConvForward<Dtype><<<CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS>>>(
          count, bottom_data, bottom[i]->num(), channels_, height_, width_,
          conved_height, conved_weight, kernel_h_, kernel_w_, stride_h_,
          stride_w_, pad_h_, pad_w_, top_data, weight, 0, bias_term_);
    }
  }
}
...
template <typename Dtype>
__global__ void
ConvBackward(const int nthreads, const Dtype *const top_diff, const int num,
             const int channels, const int height, const int width,
             const int conved_height, const int conved_width,
             const int kernel_h, const int kernel_w, const int stride_h,
             const int stride_w, const int pad_h, const int pad_w,
             Dtype *const bottom_diff, const Dtype *const weight) {
  ...
  CUDA_KERNEL_LOOP(index, nthreads) {
    ...
    const int khstart =
        (h >= kernel_h) ? ((h - kernel_h) % stride_h) + (kernel_h - stride_h)
                        : h;
    const int kwstart =
        (w >= kernel_w) ? ((w - kernel_w) % stride_w) + (kernel_w - stride_w)
                        : w;
    ...
    for (int ph = phstart; ph < phend; ++ph) {
      for (int pw = pwstart; pw < pwend; ++pw) {
        ...
        gradient += top_diff_slice[ph * conved_width + pw] *
                    weight_slice[kh * kernel_w + kw];

        //						if (index==2) {
        //							printf("pos:ph%d pw%d kh%d
        //kw%d\n",ph,pw,kh,kw); 							printf("cal:top_diff%f
        //weight%f\n",top_diff_slice[ph * conved_width +
        //pw],weight_slice[kh*kernel_w+kw]);
        //				//			printf("cal:top_diff%f
        //weight%f\n",top_diff_slice[ph * conved_width +
        //pw],weight_slice[kh*kernel_w+kw]);
        //						}
      }
...
__global__ void ConvBackwardWeight(
  ...
  CUDA_KERNEL_LOOP(index, nthreads) {
    ...
    for (int n = 0; n < num; n++) {
      ...
      const int phend =
          min(DIVIDE_CEIL((height + pad_h - kh), stride_h), conved_height);

      const int pwstart = max(DIVIDE_CEIL((pad_w - kw), stride_w), 0);

      const int pwend =
          min(DIVIDE_CEIL((width + pad_w - kw), stride_w), conved_width);
      //			if (index==5) {
      //				printf("phstart%d phend%d pwstart%d
      //pwend%d \n",phstart,phend,pwstart,pwend);
      //			}
      //
      for (int ph = phstart; ph < phend; ph++) {
        for (int pw = pwstart; pw < pwend; pw++) {
          ...
          gradient += top_diff_slice[ph * conved_width + pw] *
                      bottom_data_slice[h * width + w];
          //					if (index==5) {
          //						printf("n%d h%d w%d ph%d pw%d topdiff%f
          //bottomdata%f\n",n,h,w,ph,pw,top_diff_slice[ph * conved_width +
          //pw],bottom_data_slice[h*width+w]);
          //			//			printf("phstart%d phend%d pwstart%d
          //pwend%d \n",phstart,phend,pwstart,pwend);
          //					}
        }
...
template <typename Dtype>
__global__ void
ConvBackwardBias(const int nthreads, const Dtype *const top_diff, const int num,
                 const int channels, const int height, const int width,
                 const int conved_height, const int conved_width,
                 const int kernel_h, const int kernel_w, const int stride_h,
                 const int stride_w, const int pad_h, const int pad_w,
...
void DepthwiseConvolutionLayer<Dtype>::Backward_gpu(
  ...
  //	        << "The backward of the net whose stride is bigger than 1 is not
  //implemented now. "; 	CHECK_EQ(stride_w_, 1)
  //	        << "The backward of the net whose stride is bigger than 1 is not
  //implemented now. ";
  ...
  for (int i = 0; i < top.size(); ++i) {
    ...
    if (bias_term_ && bias_propagate_down_) {
      ...
      ConvBackwardBias<Dtype>
          <<<CAFFE_GET_BLOCKS(count_bias), CAFFE_CUDA_NUM_THREADS>>>(
              count_bias, top_diff, bottom[i]->num(), channels_, height_,
              width_, conved_height, conved_weight, kernel_h_, kernel_w_,
              stride_h_, stride_w_, pad_h_, pad_w_, bias_diff);
    }
    // gradient w.r.t. weight. Note that we will accumulate diffs.
    if (weight_propagate_down_) {
      const int count_weight = channels_ * kernel_h_ * kernel_w_;
      ConvBackwardWeight<Dtype>
          <<<CAFFE_GET_BLOCKS(count_weight), CAFFE_CUDA_NUM_THREADS>>>(
              count_weight, top_diff, bottom[i]->num(), channels_, height_,
              width_, conved_height, conved_weight, kernel_h_, kernel_w_,
              stride_h_, stride_w_, pad_h_, pad_w_, weight_diff, bottom_data);
    }
    // gradient w.r.t. bottom data, if necessary.
    if (propagate_down[i]) {
      const int count_bottom = bottom[i]->count();
      ConvBackward<Dtype>
          <<<CAFFE_GET_BLOCKS(count_bottom), CAFFE_CUDA_NUM_THREADS>>>(
              count_bottom, top_diff, bottom[i]->num(), channels_, height_,
              width_, conved_height, conved_weight, kernel_h_, kernel_w_,
              stride_h_, stride_w_, pad_h_, pad_w_, bottom_diff, weight);
    }
  }
}
