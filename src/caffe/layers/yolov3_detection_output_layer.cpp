...
void Yolov3DetectionOutputLayer<Dtype>::Forward_cpu(
            ...
            for (int c = 0; c < len; ++c) {
              ...
              if (gaussian_box_) {
                if (c == 4 || c == 6) {
                  ...
                  if (c > 7) {
                    //LOG(INFO) << c - 5;
                    class_score[c - 8] = logistic_activate(input_data[index2 + 0]);
                  } else {
  ...
  if (num_kept == 0) {
    ...
    for (int i = 0; i < num_kept; i++) {
      ...
      top_data[i * 7 + 6] = bot;
      DLOG(INFO) << "Detection box" << "," << predicts_[idxes[i]].classType << "," << predicts_[idxes[i]].x << ","
                 << predicts_[idxes[i]].y << "," << predicts_[idxes[i]].w << "," << predicts_[idxes[i]].h;
    }
