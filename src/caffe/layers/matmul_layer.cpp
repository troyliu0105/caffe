...
void MatMulLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype> *> &bottom,
  ...
  if (has_batch_) {
    ...
    for (int i = 0; i < batch_size; ++i) {
      caffe_cpu_gemm(CblasNoTrans, CblasNoTrans, M_, N_, K_, Dtype(1.0),
                     a + a_inner * i, b + b_inner * i, Dtype(0.0),
                     y + y_inner * i);
    }
  } else {
    caffe_cpu_gemm(CblasNoTrans, CblasNoTrans, M_, N_, K_, Dtype(1.0), a, b,
                   Dtype(0.0), y);
  }
}
template <typename Dtype>
void MatMulLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype> *> &top,
  ...
  if (has_batch_) {
    ...
    int b_inner = bottom[1]->count(1);
    for (int i = 0; i < batch_size; ++i) {
      caffe_cpu_gemm(CblasNoTrans, CblasTrans, M_, K_, N_, Dtype(1.0),
                     top_diff + y_inner * i, b_data + b_inner * i, Dtype(0.0),
                     a_diff + a_inner * i);
      caffe_cpu_gemm(CblasTrans, CblasNoTrans, K_, N_, M_, Dtype(1.0),
                     a_data + a_inner * i, top_diff + y_inner * i, Dtype(0.0),
                     b_diff + b_inner * i);
    }
  } else {
    caffe_cpu_gemm(CblasNoTrans, CblasTrans, M_, K_, N_, Dtype(1.0), top_diff,
                   b_data, Dtype(0.0), a_diff);
    caffe_cpu_gemm(CblasTrans, CblasNoTrans, K_, N_, M_, Dtype(1.0), a_data,
