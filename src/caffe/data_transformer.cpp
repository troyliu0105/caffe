...
/**
 * @brief rotate a cv mat with angle
 * @param src   OpenCV mat
 * @param angle angle in degrees. Positive values mean counter-clockwise
 * rotation
 */
  ...
  if (datum.encoded()) {
    ...
    if (aug_type) {
      ...
      return Transform(cv_img, transformed_blob, crop_bbox, do_mirror,
                       policy_num, preserve_pixel_vals, preserve_annotations,
                       use_previous_mirror_value);
    }
#else
...
void DataTransformer<Dtype>::TransformAnnotation(
    ...
    RepeatedPtrField<AnnotationGroup> *transformed_anno_group_all,
    int policy_num, const int rangle) {
  ...
  if (anno_datum.type() == AnnotatedDatum_AnnotationType_BBOX ||
    ...
    for (int g = 0; g < anno_datum.annotation_group_size(); ++g) {
      ...
      for (int a = 0; a < anno_group.annotation_size(); ++a) {
        ...
        if (do_resize && param_.resize_param_size() > 0) {
        ...
        if (!do_crop || ProjectBBox(crop_bbox, resize_bbox, &proj_bbox)) {
          ...
          if (do_crop && do_resize && param_.resize_param_size() > 0) {
...
void DataTransformer<Dtype>::Transform(const cv::Mat &cv_img,
  ...
  Dtype *mean = nullptr;
  if (has_mean_file &&
      !preserve_pixel_vals) { // XXX beniz: doesn't work with bbox since the
                              // image is already cropped according to bbox, use
                              // mean values instead
    CHECK_EQ(img_channels, data_mean_.channels());
    mean = data_mean_.mutable_cpu_data();
  }
  ...
  cv::Mat cv_resized_image, cv_distort_image, cv_geometry_image,
      cv_noised_image, cv_cropped_image;
  ...
  // TODO add rotate
  // if (param_.rotate() && !preserve_annotations &&
  //    phase_ == TRAIN) // deactivated with annotated datum
  //{
  //  int r = 0;
...
template <typename Dtype>
void DataTransformer<Dtype>::GeometryImage(
    const AnnotatedDatum &anno_datum, AnnotatedDatum *geometry_anno_datum) {
  const Datum &datum = anno_datum.datum();
  ...
  if (datum.encoded()) {
    ...
    cv::Mat geom_img;
    ApplyGeometry(cv_img, geom_img, anno_datum, *geometry_anno_datum,
                  param_.geometry_param());

    if (geom_img.empty()) {
      LOG(WARNING)
          << "failed geometry augmentation on image"; // no image name since
                                                      // from datum
      geom_img = cv_img;
    }
