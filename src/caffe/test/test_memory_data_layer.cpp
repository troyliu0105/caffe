...
cpu_data()[this->batch_size_ * batch_num + j]
...
datum_vector[i].
set_data(&(pixels[0]), count
);
}
layer.
AddDatumVector(datum_vector);
...
static_cast
<uint8_t>(data_string[data_index])
),
data[index++]);
}
}
...
layer.
AddMatVector(mat_vector, label_vector
);
