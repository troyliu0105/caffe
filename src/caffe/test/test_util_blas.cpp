...
TYPED_TEST(GemmTest, TestGemmCPUGPU) {
  ...
  if (sizeof(TypeParam) == 4 || CAFFE_TEST_CUDA_PROP.major >= 2) {
    ...
    caffe_blas_gemm<TypeParam>(CblasNoTrans, CblasNoTrans, 2, 4, 3, 1.,
    ...
    caffe_blas_gemm<TypeParam>(CblasTrans, CblasNoTrans, 2, 4, 3, 1.,
                               A.cpu_data(), B.cpu_data(), 0.,
                               C.mutable_cpu_data());
    for (int i = 0; i < 8; ++i) {
      EXPECT_EQ(C.cpu_data()[i], result[i]);
    ...
    caffe_blas_gemm<TypeParam>(CblasTrans, CblasTrans, 2, 4, 3, 1.,
                               A.cpu_data(), B.cpu_data(), 0.,
                               C.mutable_cpu_data());
    for (int i = 0; i < 8; ++i) {
      EXPECT_EQ(C.cpu_data()[i], result[i]);
    ...
    A.Reshape(1, 1, 2, 3);
    caffe_copy(6, data, A.mutable_cpu_data());
    caffe_blas_gemm<TypeParam>(CblasNoTrans, CblasTrans, 2, 4, 3, 1.,
                               A.cpu_data(), B.cpu_data(), 0.,
                               C.mutable_cpu_data());
    for (int i = 0; i < 8; ++i) {
      EXPECT_EQ(C.cpu_data()[i], result[i]);
...
TYPED_TEST(GemmTest, TestGemvCPUGPU) {
  ...
  if (sizeof(TypeParam) == 4 || CAFFE_TEST_CUDA_PROP.major >= 2) {
    caffe_blas_gemv<TypeParam>(CblasNoTrans, 2, 3, 1., A.cpu_data(),
    ...
    caffe_blas_gemv<TypeParam>(CblasTrans, 2, 3, 1., A.cpu_data(), y.cpu_data(),
