...
i<count;
++i) {
EXPECT_NEAR(data[i], in_data_a[i]
* in_data_b[i], 1e-5);
}
}
...
typedef typename TypeParam::Dtype Dtype;
this->blob_top_vec_[0] = this->
blob_bottom_;  // in-place computation
...
for (
int i = 0;
i < this->blob_bottom_eltwise_->

count();

++i) {
EXPECT_NEAR(orig_scale_diff
.

cpu_diff()[i],

this->blob_bottom_eltwise_->
...
for (
int i = 0;
i < this->blob_bottom_broadcast_1_->
