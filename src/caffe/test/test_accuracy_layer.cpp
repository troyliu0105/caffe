...
1);
EXPECT_EQ(this->blob_top_per_class_->
...
if (max_id == this->blob_bottom_label_->
data_at(i,
0, 0, 0)) {
++
num_correct_labels;
...
layer_param.mutable_accuracy_param()->
set_ignore_label(kIgnoreLabelValue);
AccuracyLayer <Dtype> layer(layer_param);
// Manually set some labels to the ignore label value (-1).
this->blob_bottom_label_->

mutable_cpu_data()[2] = kIgnoreLabelValue;

this->blob_bottom_label_->

mutable_cpu_data()[5] = kIgnoreLabelValue;

this->blob_bottom_label_->

mutable_cpu_data()[32] = kIgnoreLabelValue;
...
int i = 0;
i < 100; ++i) {
if (kIgnoreLabelValue == this->blob_bottom_label_->
data_at(i,
0, 0, 0)) {
continue;
}
...
EXPECT_EQ(count,
97);  // We set 3 out of 100 labels to kIgnoreLabelValue.
EXPECT_NEAR(this->blob_top_->data_at(0, 0, 0, 0),
num_correct_labels /
Dtype(count),
1e-4);
...
++num_per_class[this->blob_bottom_label_->
data_at(i,
0, 0, 0)];
if (max_id == this->blob_bottom_label_->
data_at(i,
...
EXPECT_NEAR(this->blob_top_per_class_->
data_at(i,
0, 0, 0),
