...
expected_top_shape[2] = this->
num_output_;
EXPECT_TRUE(this->blob_top_.
...
this->
ReshapeBlobs(kNumTimesteps, num
);
...
n<num;
++n) {
this->blob_bottom_cont_.

mutable_cpu_data()[t * num + n] = t > 0;
...
<< "Calling forward for full sequence LSTM";
...
t<kNumTimesteps;
++t) {
caffe_copy(bottom_count, bottom_copy
.
...
mutable_cpu_data()[n] = t > 0;

}
LOG(INFO)

<< "Calling forward for LSTM timestep " <<
t;
layer->Forward(this->blob_bottom_vec_, this->blob_top_vec_);
for (
int i = 0;
...
++i) {
ASSERT_LT(t
* top_count + i, top_copy.

count()

);
EXPECT_NEAR(this->blob_top_.
...
cpu_data()[t * top_count + i], kEpsilon
...
ASSERT_EQ(num_axes,
this->unit_blob_top_c_.

num_axes()

);
ASSERT_EQ(num_axes,
this->unit_blob_top_h_.
