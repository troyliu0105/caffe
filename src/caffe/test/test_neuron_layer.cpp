...
++i) {
EXPECT_GE(top_data[i],
0.);
EXPECT_TRUE(top_data[i]
== 0 || top_data[i] == bottom_data[i]);
}
}
...
++l) {
EXPECT_GE(this->blob_top_->
data_at(i, j, k, l
) + 1e-4,
(exp(2 * this->blob_bottom_->
data_at(i, j, k, l
)) - 1) /
(exp(2 * this->blob_bottom_->
data_at(i, j, k, l
)) + 1));
EXPECT_LE(this->blob_top_->
data_at(i, j, k, l
) - 1e-4,
(exp(2 * this->blob_bottom_->
data_at(i, j, k, l
...
this->
TestExpForward(kBase, kScale, kShift
);
}
...
const Dtype kShift = 0;
this->
TestExpGradient(kBase, kScale, kShift
);
}
...
const Dtype kShift = 0;
this->
TestLogForward(kBase, kScale, kShift
);
}
...
const Dtype kShift = 0;
this->
TestLogGradient(kBase, kScale, kShift
);
}
...
this->
TestDropoutForward(kDropoutRatio);
}
...
relu.
SetUp(blob_bottom_vec_2, blob_top_vec_2
);
// Check forward
...
relu.
Backward(blob_top_vec_2, propagate_down, blob_bottom_vec_2
);
for (
int s = 0;
s<blob_bottom_2->
...
++s) {
EXPECT_EQ(this->blob_bottom_->

cpu_diff()[s], blob_bottom_2
...
ip2.
SetUp(blob_bottom_vec_2, blob_middle_vec_2
);
prelu2.
SetUp(blob_middle_vec_2, blob_top_vec_2
);
caffe_copy(ip2
.blobs()[0]->count(), ip.blobs()[0]->cpu_data(),
...
ip2.
Forward(blob_bottom_vec_2, blob_middle_vec_2
);
prelu2.
Forward(blob_middle_vec_2, blob_top_vec_2
);
// Check numbers
for (
int s = 0;
...
prelu2.
Backward(blob_top_vec_2, propagate_down, blob_middle_vec_2
);
ip2.
Backward(blob_middle_vec_2, propagate_down, blob_bottom_vec_2
);
// Check numbers
for (
int s = 0;
...
++s) {
EXPECT_FLOAT_EQ(this->blob_bottom_->
...
for (
int s = 0;
s<prelu.blobs()[0]->count(); ++s) {
EXPECT_FLOAT_EQ(prelu
.blobs()[0]->cpu_diff()[s],
