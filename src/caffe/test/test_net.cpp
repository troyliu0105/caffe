...
EXPECT_EQ(this->net_->blob_by_name("data"), this->net_->

blobs()[0]

);
EXPECT_EQ(this->net_->blob_by_name("label"), this->net_->

blobs()[1]

);
EXPECT_EQ(this->net_->blob_by_name("innerproduct"), this->net_->

blobs()[2]

);
EXPECT_FALSE(this->net_->blob_by_name("loss"));
EXPECT_EQ(this->net_->blob_by_name("top_loss"), this->net_->
...
EXPECT_EQ(this->net_->layer_by_name("data"), this->net_->

layers()[0]

);
EXPECT_EQ(this->net_->layer_by_name("innerproduct"), this->net_->

layers()[1]

);
EXPECT_EQ(this->net_->layer_by_name("loss"), this->net_->
...
EXPECT_EQ(3, bottom_need_backward.

size()

);
EXPECT_EQ(0, bottom_need_backward[0].

size()

);
EXPECT_EQ(1, bottom_need_backward[1].

size()

);
EXPECT_EQ(false, bottom_need_backward[1][0]);
EXPECT_EQ(2, bottom_need_backward[2].
...
const bool force_backward = true;
this->
InitTinyNetEuclidean(force_backward);
const vector <vector<bool>> &bottom_need_backward =
    this->net_->bottom_need_backward();
EXPECT_EQ(3, bottom_need_backward.
...
const vector <vector<bool>> &bottom_need_backward =
    this->net_->bottom_need_backward();
EXPECT_EQ(4, bottom_need_backward.
...
EXPECT_EQ(false, bottom_need_backward[1][0]);
EXPECT_EQ(1, bottom_need_backward[2].

size()

);
EXPECT_EQ(false, bottom_need_backward[2][0]);
EXPECT_EQ(2, bottom_need_backward[3].
...
blob_grads;
this->
CopyNetBlobs(kCopyDiff, &blob_grads
);
vector <shared_ptr<Blob < Dtype>> >
param_grads;
this->
CopyNetParams(kCopyDiff, &param_grads
);
...
++j) {
ASSERT_EQ(blob_grads[j]
->

count(), weighted_blobs[j]
...
weighted_params =
this->net_->params();
ASSERT_EQ(param_grads
.

size(), weighted_params
...
++j) {
ASSERT_EQ(param_grads[j]
->

count(), weighted_params[j]
...
this->
InitUnsharedWeightsNet(&loss_weight, &midnet_loss_weight,
    kForceBackward
);
const Dtype loss = this->net_->ForwardBackward();
const bool kCopyDiff = true;
...
Caffe::set_random_seed(this->seed_);
this->
InitUnsharedWeightsNet(&loss_weight, &kLossWeights[i],
    kForceBackward
);
const Dtype weighted_loss = this->net_->ForwardBackward();
const Dtype error_margin = kErrorMargin * fabs(kLossWeights[i]);
...
blob_grads_loss_2;
this->
CopyNetBlobs(kCopyDiff, &blob_grads_loss_2
);
vector <shared_ptr<Blob < Dtype>> >
param_grads_loss_2;
this->
CopyNetParams(kCopyDiff, &param_grads_loss_2
);
...
EXPECT_GT(fabs(grad_diff_2), kMinGradDiffAbsValue
) <<
blob_name;
EXPECT_NEAR(2 * grad_diff_2, grad_diff_3, kErrorMargin) <<
blob_name;
} else {
EXPECT_EQ(0, grad_diff_2) <<
blob_name;
EXPECT_EQ(0, grad_diff_3) <<
blob_name;
}
}
}
...
const vector <shared_ptr<Blob < Dtype>> > &
blob_grads_midnet_loss_3 =
this->net_->blobs();
ASSERT_EQ(blob_grads
.

size(), blob_grads_midnet_loss_3
...
count(), blob_grads_midnet_loss_3[j]
...
// locations.
EXPECT_EQ(ip1_weights
->

cpu_data(), ip2_weights
...
shared_params.
CopyFrom(*ip1_weights, copy_diff, reshape
);
shared_params.
CopyFrom(*ip1_weights,
!copy_diff, reshape);
...
i<count;
++i) {
EXPECT_NE(0, ip1_weights->

cpu_diff()[i]

);
}
caffe_axpy(count, Dtype(-1), shared_params
.
...
unshared_params2.
CopyFrom(*ip2_weights, copy_diff, reshape
);
unshared_params2.
CopyFrom(*ip2_weights,
!copy_diff, reshape);
...
caffe_axpy(count, Dtype(-1), ip1_weights
->
...
caffe_axpy(count, Dtype(-1), ip2_weights
->
...
shared_params.
CopyFrom(*ip1_weights, kCopyDiff, kReshape
);
const int count = ip1_weights->count();
...
this->net_->
CopyTrainedLayersFrom(net_param);
ip1_weights = this->net_->layers()[1]->blobs()[0].get();
ip2_weights = this->net_->layers()[2]->blobs()[0].get();
...
i<count;
++i) {
EXPECT_FLOAT_EQ(shared_params
.

cpu_data()[i], ip1_weights
...
this->
InitUnsharedWeightsNet(kLossWeight1, kLossWeight2, kForceBackward,
...
const vector <shared_ptr<Blob < Dtype>> > &
params2 = this->net_->params();
ASSERT_EQ(num_params, params2
.
...
const vector <shared_ptr<Blob < Dtype>> > &
params3 = this->net_->params();
ASSERT_EQ(num_params, params3
.
...
const vector <shared_ptr<Blob < Dtype>> > &
params4 = this->net_->params();
ASSERT_EQ(num_params, params4
.
