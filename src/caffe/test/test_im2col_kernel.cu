...
template <typename Dtype> class Im2colKernelTest : public GPUDeviceTest<Dtype> {
protected:
  Im2colKernelTest()
      // big so launches > 1024 threads
      : blob_bottom_(new Blob<Dtype>(5, 500, 15, 15)),
        blob_kernel_shape_(new Blob<int>()), blob_stride_(new Blob<int>()),
        blob_pad_(new Blob<int>()), blob_dilation_(new Blob<int>()),
        blob_top_(new Blob<Dtype>()), blob_top_cpu_(new Blob<Dtype>()) {
    ...
    kernel_size_ = 3;
    height_col_ =
        (height_ + 2 * pad_ - (dilation_ * (kernel_size_ - 1) + 1)) / stride_ +
        1;
    width_col_ =
        (width_ + 2 * pad_ - (dilation_ * (kernel_size_ - 1) + 1)) / stride_ +
        1;
...
TYPED_TEST(Im2colKernelTest, Test2D) {
  ...
  this->blob_top_->Reshape(this->blob_bottom_->num(),
                           this->channels_ * this->kernel_size_ *
                               this->kernel_size_,
  ...
  for (int n = 0; n < this->blob_bottom_->num(); ++n) {
    im2col_cpu(this->blob_bottom_->cpu_data() + this->blob_bottom_->offset(n),
               ...
               this->stride_, this->dilation_, this->dilation_,
  ...
  for (int grid_div = 2; grid_div <= 8; grid_div++) {
    for (int n = 0; n < this->blob_bottom_->num(); ++n) {
      ...
      im2col_gpu_kernel<TypeParam><<<grid_dim, CAFFE_CUDA_NUM_THREADS>>>(
          ...
          this->pad_, this->pad_, this->stride_, this->stride_, this->dilation_,
          this->dilation_, this->height_col_, this->width_col_,
