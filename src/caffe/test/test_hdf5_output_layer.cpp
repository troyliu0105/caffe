...
protected:
  HDF5OutputLayerTest()
      : input_file_name_(ABS_TEST_DATA_DIR "/sample_data.h5"),
        blob_data_(new Blob<Dtype>()), blob_label_(new Blob<Dtype>()), num_(5),
        channels_(8), height_(5), width_(5) {
...
TYPED_TEST(HDF5OutputLayerTest, TestForward) {
  ...
  hid_t file_id =
      H5Fopen(this->input_file_name_.c_str(), H5F_ACC_RDONLY, H5P_DEFAULT);
  ...
  file_id =
      H5Fopen(this->output_file_name_.c_str(), H5F_ACC_RDONLY, H5P_DEFAULT);
  ASSERT_GE(file_id, 0) << "Failed to open HDF5 file" << this->input_file_name_;
