...
for (
int j = 0;
j < this->blob_top_vec_[i]->

count();

++j) {
EXPECT_EQ(0, this->blob_top_vec_[i]->
...
++j) {
EXPECT_EQ(7, this->blob_top_vec_[i]->
...
data_filler_param_b->
set_mean(gaussian_mean);
data_filler_param_b->
set_std(gaussian_std);
...
EXPECT_NE(first_gaussian_sample,
this->blob_top_b_->
...
cpu_data()[this->blob_top_b_->count() - 1]
