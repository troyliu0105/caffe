...
layer.reset(new
ConvolutionLayer<Dtype>(layer_param)
);
layer->SetUp(this->blob_bottom_vec_, this->blob_top_vec_);
EXPECT_EQ(this->blob_top_->
...
caffe_conv(this->blob_bottom_, convolution_param, layer->
...
++i) {
EXPECT_NEAR(top_data[i], ref_top_data[i],
1e-4);
}
caffe_conv(this->blob_bottom_2_, convolution_param, layer->
...
for (
int i = 0;
i < this->blob_bottom_vec_.

size();

++i) {
this->blob_bottom_vec_[i]->
Reshape(bottom_shape);
}
LayerParameter layer_param;
ConvolutionParameter *convolution_param =
    layer_param.mutable_convolution_param();
...
bottom_d;
value += weight->
data_at(weight_offset)
*
this->blob_bottom_->

cpu_data()[n * bottom_dim + bottom_d];
...
filler_param.set_value(1.);
filler.reset(new
GaussianFiller<Dtype>(filler_param)
);
filler->Fill(this->blob_bottom_);
...
layer->
SetUp(sep_blob_bottom_vec, sep_blob_top_vec
);
layer->
Forward(sep_blob_bottom_vec, sep_blob_top_vec
);
...
++i) {
EXPECT_NEAR(top_data[i], sep_top_data[i],
1e-4);
}
}
...
convolution_param->
set_kernel_h(kernel_h);
convolution_param->
set_kernel_w(kernel_w);
convolution_param->mutable_weight_filler()->set_type("gaussian");
Blob <Dtype> weights;
Blob <Dtype> top_diff;
...
reshape = false;
layer_2d.blobs()[0]->
CopyFrom(weights, copy_diff, reshape
);
...
ASSERT_EQ(backward_result_nd
.

count(), backward_result_2d
...
for (
int i = 0;
i<backward_result_2d.

count();

++i) {
EXPECT_FLOAT_EQ(backward_result_2d
.
...
ASSERT_EQ(backward_weight_result_nd
.
...
for (
int i = 0;
i<backward_weight_result_2d.

count();

++i) {
EXPECT_EQ(backward_weight_result_2d
.
