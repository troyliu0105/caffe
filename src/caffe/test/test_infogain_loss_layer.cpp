...
#include "caffe/layers/loss/infogain_loss_layer.hpp"
...
protected:
  InfogainLossLayerTest()
      : blob_bottom_data_(new Blob<Dtype>(4, 2, 5, 2)),
        ...
        blob_top_loss_(new Blob<Dtype>()), blob_top_prob_(new Blob<Dtype>()),
    ...
    for (int j = 0; j < this->inner_; j++) {
      ...
      for (int l = 0; l < this->num_labels_; l++) {
        loss -= H[gt * this->num_labels_ + l] *
                log(std::max(prob[i * this->num_labels_ * this->inner_ +
                                  l * this->inner_ + j],
