      ...
      if (j == 0) {
        ...
        for (int k = 0; k < num_det; ++k) {
          ...
          if (item_id == -1) {
            ...
            int fp = static_cast<int>(result_vec[k * 5 + 4]);
            if (tp == 0 && fp == 0) {
              // Ignore such case. It happens when a detection bbox is matched
              // to a difficult gt bbox and we don't evaluate on difficult gt
              // bbox.
              continue;
            }
            all_true_pos[j][label].push_back(std::make_pair(score, tp));
        ...
        // LOG(INFO)<<result[1]->width()<<" , "<<result[1]->height()<<" ,
        // "<<result[1]->num()<<" , "<<result[1]->channels();
