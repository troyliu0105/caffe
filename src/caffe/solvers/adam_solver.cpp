...
void AdamSolver<Dtype>::ComputeUpdateValue(int param_id, Dtype rate) {
  ...
  case Caffe::CPU: {
    ...
    caffe_blas_axpby(N, Dtype(1) - beta1, net_params[param_id]->cpu_diff(),
    ...
    caffe_blas_axpby(N, Dtype(1) - beta2, val_t->cpu_data(), beta2,
    ...
    caffe_blas_scale(N, local_rate * correction, val_t->cpu_data(),
  ...
  case Caffe::GPU: {
    ...
    adam_update_gpu(N, net_params[param_id]->mutable_gpu_diff(),
                    val_m->mutable_gpu_data(), val_v->mutable_gpu_data(), beta1,
                    beta2, eps_hat, local_rate * correction);
#else
    NO_GPU;
#endif
    break;
