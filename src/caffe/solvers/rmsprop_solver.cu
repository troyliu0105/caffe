...
__global__ void RMSPropUpdate(int N, Dtype *g, Dtype *h, Dtype rms_decay,
...
void rmsprop_update_gpu(int N, Dtype *g, Dtype *h, Dtype rms_decay, Dtype delta,
  ...
  RMSPropUpdate<Dtype> // NOLINT_NEXT_LINE(whitespace/operators)
      <<<CAFFE_GET_BLOCKS(N), CAFFE_CUDA_NUM_THREADS>>>(N, g, h, rms_decay,
                                                        delta, local_rate);
  CUDA_POST_KERNEL_CHECK;
}
template void rmsprop_update_gpu<float>(int, float *, float *, float, float,
                                        float);
template void rmsprop_update_gpu<double>(int, double *, double *, double,
                                         double, double);
